...
```
NewKet/
├── ADMIN NewKet/       # Interface d'Administration
...
Le dossier `ADMIN NewKet` contient les outils pour gérer le site. Grâce au partage du `localStorage`, les modifications faites ici se reflètent immédiatement sur le site client.
...
2.  **Administration :** Pour accéder au back-office, ouvrez `ADMIN NewKet/dashboard.html` (ou via le lien "Compte" -> Admin si configuré).
